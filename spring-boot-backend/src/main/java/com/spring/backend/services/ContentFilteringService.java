...
public class ContentFilteringService {
    ...
    double getFeatureSimilarity(Hotel hotel,Long userId){
        Double[] hotelProfile = getHotelProfile(hotel);
        return cosineSimilarity(buildUserProfile(userId), hotelProfile);
    }

    double getTextSimilarity(Hotel hotel,Long userId){
        String userDescription = buildUserDescription(userId);
        Map<String, Double> idfScores = calculateIdfScores(hotelRepository.findAll());
        return cosineSimilarityText(idfScores, userDescription, hotel.getDescription());
    }
